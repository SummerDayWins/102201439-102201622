<template>
  <view class="container">
    <text class="title">我加入的项目</text>
    <view class="project-list">
      <view class="project-card" v-for="project in joinedProjects" :key="project.id">
      ...
      <text v-if="joinedProjects.length === 0" class="no-projects">您还没有加入任何项目。</text>
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    removeProject(projectId) {
      // 从当前列表中移除项目
      this.joinedProjects = this.joinedProjects.filter(project => project.id !== projectId);
      
      // 更新本地存储
      localStorage.setItem('joinedProjects', JSON.stringify(this.joinedProjects));
